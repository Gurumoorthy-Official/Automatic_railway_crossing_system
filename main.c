...
void ServoOpen(void)
{
    ...
    for (int32_t p = current; p >= target; p -= 2)
    {
...
int main(void)
{
    ...
    // Start servo PWM and set gate open at start
...
void SensorTask(void *argument)
{
    osDelay(2000); // to ignore unstable startup readings
...
//led task to check system running or not
void LedTask(void *argument)
{
    for (;;)
    {
...
{
    __disable_irq();
    HAL_GPIO_WritePin(ERROR_LED_GPIO_Port, ERROR_LED_Pin, GPIO_PIN_SET); // switch on the led on pin PB12
    while (1)
    {
